...
| `skill.py` | 技能系统（v0.1.53 从核心移出）：`use_skill` 加载 `SKILL.md` + scripts/references；`skills_guidance` 动态段列出运行时技能清单（静态规则经工具 guidance 注入）<br>Skill system (moved out of core in v0.1.53): `use_skill` loads `SKILL.md` + scripts/references; dynamic `skills_guidance` section lists runtime skills (static rules via tool guidance) | 无 none | `tools` + `prompt_sections()` |
